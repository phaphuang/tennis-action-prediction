...
class TennisVideoDataset(VarLenFeatureVideoDataset):
    def __init__(self, *args, **kwargs):
        super(TennisVideoDataset, self).__init__(*args, **kwargs)
        self.state_like_names_and_shapes['images'] = 'images/encoded', (64, 64, 3)

    def get_default_hparams_dict(self):
        default_hparams = super(TennisVideoDataset, self).get_default_hparams_dict()
        hparams = dict(
            context_frames=10,
            sequence_length=20,
...
def partition_data(input_dir):
    ...
    print(files)
    persons = np.array([int(f.split('/raw\\')[1].split('-')[0]) for f in files])
